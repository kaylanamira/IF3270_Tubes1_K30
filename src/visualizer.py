...
class Visualizer:
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            for neuron_idx, neuron in enumerate(layer.neurons):
                ...
                for prev_neuron_idx in range(neuron.n_in):
                    ...
                    self.graph.add_edge(prev_neuron_id, neuron_id, weight=round(neuron.w[prev_neuron_idx].data, 2))

                if (layer_idx>0):
                    neuron_bias_id = f"B{layer_idx}"
                    self.graph.add_edge(neuron_bias_id, neuron_id, weight=round(neuron.b.data, 2))
    ...
    def plot_neural_network(self):
        ...
        edge_labels = {(src, dest): data["weight"] for src, dest, data in self.graph.edges(data=True)}
